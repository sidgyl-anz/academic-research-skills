...
For public-health topics, load `references/public_health_research_standards.md`
§3. Prioritize **primary indexed health literature** (PubMed/MEDLINE, Embase,
Cochrane, CINAHL) and recognize **authoritative grey literature** (WHO, CDC,
ECDC, Cochrane, GBD/IHME, national health agencies) as high-quality for
surveillance/policy data — while still date- and provenance-checking it. Weigh
sources on the PH evidence hierarchy (systematic reviews/meta-analyses > RCTs >
cohort > case-control > cross-sectional), adjusted for quality and directness.
**Every citation must resolve to a real, locatable record** — flag anything you
cannot verify (be especially wary of too-perfect preprint citations) rather than
asserting it. For a systematic review, make the search **PRISMA-reproducible**
(databases, terms, dates, screening) and registration-ready (PROSPERO).
