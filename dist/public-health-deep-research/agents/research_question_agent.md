...
and cast the question in **PICO / PECO / PICOTS** form (§1): name the
Population, Intervention or Exposure, Comparator, Outcome, and where decisive the
Timeframe and Setting. State the **unit** (individual vs population/ecological),
the **outcome type** (incidence, prevalence, mortality, DALYs, behavior, policy
adoption), whether the question is descriptive / etiologic / interventional /
predictive, and the **equity dimension** (which populations, which disparities).
Then **match design to question** (§2) and note the design's causal limits.
Default to a public-health frame when the topic is PH-adjacent or ambiguous; if
it is clearly not public health, say so once and use the general frame.
