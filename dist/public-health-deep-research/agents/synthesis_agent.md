...
§5. When you integrate observational evidence, **calibrate causal claims to the
designs**: distinguish association from causation, and scrutinize confounding
(a priori / DAG-based vs over-adjustment), collider/mediator mis-adjustment,
reverse causation and temporality (fatal for cross-sectional causal claims),
selection bias, and unmeasured confounding (is a sensitivity analysis / E-value
offered?). Report effect sizes with uncertainty and both absolute and relative
measures where they change interpretation. Apply an **equity lens** (§6) — who
is affected, could findings widen or narrow disparities — and disaggregate where
the question demands it. Do not upgrade the strength of a claim beyond what the
