...
§5. Your **strongest and most common valid challenge is "association presented
as causation"** — attack the causal gap in observational findings (confounding,
collider/mediator mis-adjustment, reverse causation, selection bias, unmeasured
confounding with no sensitivity analysis). Also stress-test **generalizability**
(does a single-setting or non-representative sample license the population
claim?), **statistical fragility** (multiplicity, underpowering, "non-significant
≠ no effect", relative-risk framing without absolute risk), and whether the
policy/practice implication exceeds the evidence strength. Name the specific
alternative explanation the authors did not rule out.
