...
# Public Health Deep Research — Academic Research Agent Team

Universal deep research tool — a 13-agent team for rigorous academic research,
**tuned in this build to public health by default** (it still researches any
topic; absent a clear signal it assumes a public-health question).

> **Standalone public-health build.** This is the `deep-research` engine,
> packaged as a single self-contained skill for upload to Claude.ai, with public
> health baked in as the default domain via
> `references/public_health_research_standards.md`.

## Running on Claude.ai (read first)

This skill was originally built for Claude Code, which wraps it in hooks, Python
gate scripts, CI linters, `ARS_*` environment variables, live bibliographic-API
lookups, and an optional cross-model verifier. **On Claude.ai those layers
degrade to advisory prose** — the research reasoning is driven by the
instructions here. Specifically:

- **Deterministic gates degrade to advisory.** `scripts/*.py`, PreToolUse
  guards, and CI describe the Claude Code path; here the agents follow the same
  logic as instructions, with no separate automated checker.
- **`ARS_*` environment variables are inert** (`ARS_CROSS_MODEL`,
  `ARS_MODEL_TIERING`, `ARS_PASSPORT_RESET`, the Socratic probe flags, etc.).
- **Live literature lookups depend on tools being enabled.** The arXiv /
  OpenAlex / Crossref / Semantic Scholar / PubMed protocols in `references/`
  describe programmatic verification. On Claude.ai, real lookups happen only if
  web search / a connector (e.g. PubMed) is enabled; otherwise the team reasons
  from provided/known sources and must **flag any citation it could not verify
  against a real record** rather than asserting it.
- **Not every referenced path is bundled.** `./shared/` carries the files the
  research directly needs (human-subjects authority + IRB glossary,
  ground-truth isolation, evidence-row + human-subjects contracts, word-count /
  hedging references, mode spectrum). Deeper pointers — sibling skills
  (`academic-paper`, `academic-paper-reviewer`, `academic-pipeline`), repo
  paths (`scripts/`, `docs/design/`, `.claude/CLAUDE.md`), and toolkit-only
  `shared/…` files (Material Passport schemas, cross-model doc) — are provenance
  references, **not runtime loads**; if absent, ignore and proceed.
- **What fully works here:** question framing, methodology design, literature
  strategy, synthesis, risk-of-bias appraisal, meta-analysis reasoning, PRISMA
  systematic-review structure, ethics review, fact-check, Socratic guided
  research, and report compilation — all prose-driven.

## Public Health Default (this build)

- `research_question_agent` frames questions in **PICO/PECO/PICOTS** form,
  names the unit and outcome type, and surfaces equity dimensions up front. When
  the topic is public-health-adjacent **or ambiguous**, it defaults to a
  public-health frame and every agent loads
  `references/public_health_research_standards.md`.
- **Design is matched to the question** and its causal limits stated
  (observational ⇒ association, not causation, without a defended framework).
- **Evidence is weighed on the PH hierarchy** (systematic reviews/meta-analyses
  > RCTs > cohort > case-control > cross-sectional), with authoritative grey
  literature (WHO/CDC/Cochrane/GBD) recognized and every citation verified
  against a real record.
- Systematic reviews are **PROSPERO-registered, PRISMA-reported**, with the
  design-appropriate risk-of-bias tool and **GRADE** certainty.
- The **strongest devil's-advocate challenge** to a PH finding is "association
  presented as causation" — applied by default.
- If the topic is clearly NOT public health, announce the detected domain once
  and proceed on the general references (the PH standard does not fire).
