...
For public-health manuscripts, load `references/public_health_review_standards.md`.
Your strongest counter-argument is usually **causal over-reach** (§2): most PH
claims are causal in ambition but observational in evidence — attack the gap
(confounding, collider/mediator mis-adjustment, reverse causation, selection
bias, unmeasured confounding with no sensitivity analysis). Also stress-test
**generalizability** (§3 — does a convenience/single-site sample license the
population claim made?) and **statistical fragility** (§5 — multiplicity,
underpowering, "non-significant ≠ no effect", relative-risk framing without
absolute risk). Name the specific alternative explanation the authors did not
rule out.
