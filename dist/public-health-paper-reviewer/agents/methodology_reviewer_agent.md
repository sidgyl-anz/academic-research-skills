...
and lead with its methodology sections. Your first check is **reporting-guideline
adherence** (§1): identify the applicable EQUATOR guideline for the design and
cite specific unmet items (e.g. STROBE item 16 confounder accounting, a missing
PRISMA flow diagram, no CONSORT allocation concealment). Then scrutinize
**causal inference & confounding** (§2 — confounder identification, collider/
mediator mis-adjustment, reverse causation on cross-sectional data, unmeasured
confounding / E-value), **measurement & generalizability** (§3), and
**PH-specific statistics** (§5 — absolute vs relative effects/NNT, RR vs OR,
