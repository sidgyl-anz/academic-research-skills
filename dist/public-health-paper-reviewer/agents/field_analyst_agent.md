...
## Public Health Domain Default (this build)

This is the public-health build of the reviewer. Detect the paper's field as
usual, then:

- **If the manuscript is public-health-adjacent OR the domain is ambiguous**,
  default the panel to public-health expert personas and set the domain to
  public health. PH-adjacent includes epidemiology, biostatistics, health
  policy, health services research, global/community/population health,
  environmental & occupational health, implementation science, health
  promotion, and health economics.
- **Identify the applicable EQUATOR reporting guideline** for the study design
  (STROBE / CONSORT / PRISMA / SPIRIT / STARD / TRIPOD / CHEERS / RECORD / COREQ
  / StaRI — see `references/public_health_review_standards.md` §1) and name it in
  the Reviewer Configuration Card so the methodology seat checks adherence.
- Configure Reviewer personas with concrete PH expertise, e.g. R1 = an
  epidemiologist/biostatistician, R2 = a subject-matter specialist in the paper's
  health topic, R3 = a health-policy / health-equity / implementation scholar.
- Instruct every seat to load `references/public_health_review_standards.md`
  alongside the field-general criteria.
- **If the paper is clearly NOT public health**, announce the detected field
  once, configure ordinary field-matched personas, and do not fire the PH
  reference. Never force a PH lens onto a non-PH paper.
