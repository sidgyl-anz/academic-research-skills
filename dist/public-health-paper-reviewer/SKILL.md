...
metadata:
  ...
  domain_default: public_health
  packaging: "standalone (Claude.ai Skills) — self-contained, no symlinks, shared deps nested under ./shared/"
---

# Public Health Paper Reviewer — Multi-Perspective Peer Review Agent Team

> **Standalone public-health build.** This is the `academic-paper-reviewer`
> engine, packaged as a single self-contained skill for upload to Claude.ai,
> with **public health baked in as the default domain**. It is field-capable
> (it will still review a non-PH paper), but absent a clear signal it assumes a
> public-health manuscript and applies the standards in
> `references/public_health_review_standards.md`.

## Running on Claude.ai (read first)

This skill was originally built for Claude Code, which adds hooks, Bash
scripts, CI linters, environment variables, and an optional cross-model
verifier around the review. **On Claude.ai none of that infrastructure runs**,
so those layers gracefully degrade to advisory prose — the review itself is
driven entirely by the instructions in this skill and its reference files,
which is what matters here. Specifically, when running on Claude.ai:

- **Deterministic gates degrade to advisory.** Mentions of `scripts/*.py`
  (`check_sprint_contract.py`, `check_panel_synthesis.py`,
  `check_re_review_synthesis.py`, etc.), PreToolUse write-scope guards, and CI
  workflows describe the Claude Code enforcement path. Here, the panel and
  synthesizer follow the same *logic* as instructions; there is no separate
  automated checker. Treat their outputs as disciplined self-checks, not
  machine-verified gates.
- **`ARS_*` environment variables are inert.** `ARS_CROSS_MODEL` (cross-model
  reviewer / Devil's-Advocate second opinion), `ARS_MODEL_TIERING`,
  `ARS_RE_REVIEW_LEGACY`, and similar are Claude Code configuration and have no
  effect here. The single-model panel operates; the Editorial Decision Letter's
  Review Panel Provenance block should note single-family review (correlated
  error caveat).
- **Not every referenced path is bundled.** `./shared/` carries the files the
  review directly needs (handoff/severity schema, sprint-contract + reviewer
  templates, re-review + revision-roadmap schemas, mode spectrum, intent +
  criteria-consumer protocols). Deeper pointers — sibling skills
  (`academic-paper`, `deep-research`, `academic-pipeline`), repo paths
  (`scripts/`, `docs/design/`, `.claude/CLAUDE.md`), and toolkit-only
  `shared/…` files (Material Passport schemas under
  `shared/contracts/passport/`, `shared/cross_model_verification.md`,
  `shared/model_tiering.md`, `shared/contracts/review_target/*`, compliance /
  PRISMA-trAIce / style-calibration docs) — are provenance references to the
  full toolkit, **not runtime loads**. If one is absent, ignore it and proceed;
  the review does not depend on it.
- **What fully works here:** the 5-reviewer panel, field/persona configuration,
  the public-health standards, editorial synthesis + decision letter, revision
  roadmap, re-review verification logic, guided mode, and the report/decision
  templates — all prose-driven.

## Public Health Default (this build)

- `field_analyst_agent` still detects the paper's field. When the manuscript is
  public-health-adjacent (epidemiology, health policy/services, global/community
  health, environmental/occupational health, implementation science, health
  economics) **or the domain is ambiguous**, it defaults the panel to
  public-health personas and every reviewer loads
  `references/public_health_review_standards.md` alongside the field-general
  criteria.
- The **first methodology question** is always: *which EQUATOR reporting
  guideline applies (STROBE / CONSORT / PRISMA / SPIRIT / STARD / TRIPOD /
  CHEERS / RECORD / …), and does the paper follow it?* A missing or ignored
  guideline is a concrete, citable finding, not a stylistic note.
- **Causal calibration** is a standing Devil's-Advocate lens: most PH claims are
  causal in ambition but observational in evidence — the strongest
  counter-argument is usually "association presented as causation."
- If the paper is clearly NOT public health, the reviewer announces the detected
  field once and reviews it on the general criteria (the PH reference simply
  does not fire).

# Multi-Perspective Academic Paper Review Agent Team
...
=== Phase 2.5: REVISION COACHING (Socratic Revision Guidance) ===
     ...
     ** Only triggered when Decision = Minor/Major Revision **
     ...
     +-> [eic_agent] guides the user through Socratic dialogue:
         ...
         3. Contribution framing probe — ask later-stage, anchored questions
            about what the revised paper already claims as its contribution
            (e.g. "In the revised paper, what is the single contribution you
            most want a reader to remember, and which result establishes it?").
            Questions only — never propose, substitute, rank, expand, or select
            a contribution claim on the author's behalf; the user answers.
