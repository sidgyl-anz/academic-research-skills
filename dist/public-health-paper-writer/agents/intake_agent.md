...
This is the public-health build. During the interview, detect the domain from
the topic and materials. **If the topic is public-health-adjacent (epidemiology,
health policy/services, global/community/population health, environmental &
occupational health, implementation science, health economics) OR the domain is
ambiguous, default the discipline to public health.** Then:

- Determine the **study design** and record the applicable EQUATOR reporting
  guideline (STROBE / CONSORT / PRISMA / SPIRIT / STARD / TRIPOD / CHEERS /
  RECORD / COREQ / StaRI — see `references/public_health_writing_standards.md`
  §1) in the Paper Configuration Record, so the outline is built against it.
- Default the abstract to the **structured** form (Background/Objectives ·
  Methods · Results · Conclusions).
- Note in the Record that all downstream agents load
  `references/public_health_writing_standards.md`.
- **If the topic is clearly NOT public health**, record the detected discipline
  and proceed on the general references (higher-education default); do not force
  a PH frame.
