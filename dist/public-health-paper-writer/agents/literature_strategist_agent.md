...
Frame the Introduction evidence around **burden/importance** of the health
problem (magnitude, populations affected, equity relevance) and the specific gap
(§2). Weight the evidence hierarchy as public health does — systematic reviews /
meta-analyses and well-conducted cohort/RCT evidence over single small or
cross-sectional studies — and prefer primary sources indexed in the health
literature (e.g. PubMed/MEDLINE-style records) where available. When the paper
itself is a systematic review, the search strategy must be PRISMA-reproducible
(documented databases, terms, dates, and screening) and registration-ready
(PROSPERO).
