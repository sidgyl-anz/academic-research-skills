...
For public-health manuscripts, load `references/public_health_writing_standards.md`.
Write Methods and Results to the guideline conventions (§2): report the
pre-specified primary outcome first, give **both absolute and relative effect
measures with 95% CIs** (not bare p-values), label post-hoc/exploratory
analyses as such, and keep interpretation out of Results. **Calibrate causal
language to the study design (§3):** for observational data write "associated
with" / "more likely", never bare causal verbs ("causes", "reduces"); the
Abstract and Discussion conclusions must not exceed the Results, and a policy
recommendation must match the evidence strength. Honor upstream-marked
protected hedges (`shared/references/protected_hedging_phrases.md`,
`shared/references/claim_strength_ladder.md`) — do not silently strengthen or
weaken a claim across drafts. Include ethics/IRB, funding, competing-interest,
data/code-availability, and registration statements (§5), using terminology
