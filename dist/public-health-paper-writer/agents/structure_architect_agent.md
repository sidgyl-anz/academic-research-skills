...
and build the outline **against the applicable EQUATOR reporting guideline**
(§1) recorded by the intake agent. Use guideline-driven IMRaD (§2): the Methods
outline must include design, setting, participants/eligibility, variables
(exposures/outcomes/confounders with definitions), data sources/measurement,
bias handling, study size/power, and statistical methods (confounder handling,
subgroup/sensitivity, missing data); the Results outline must include
participant flow (a diagram for trials/reviews), Table 1, and main results with
both absolute and relative effect measures + 95% CIs. Reserve a **structured
