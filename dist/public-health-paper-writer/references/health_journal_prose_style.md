...
Style reference for drafting manuscripts aimed at health, medical and
digital-health journals. It answers a narrower question than
`academic_writing_style.md` (which sets general academic register) and
...
## 13. How the writing agents use this file

| Agent | Primary sections |
|---|---|
| `structure_architect_agent` | §9 Discussion architecture (subsection skeleton), §7 title, §8 abstract slots and length budget |
| `draft_writer_agent` | §1 rhythm, §2 register by section (the primary discipline), §3 hedging, §4 numbers in prose, §5 connectives, §10 limitations, §11 conclusions |
| `abstract_bilingual_agent` | §8 (header set, length, density, no claim stronger than the Discussion), §3 |
| `revision_coach_agent` | §12 self-check as the revision pass; §10 when reviewers ask for limitations to be expanded |
| `peer_reviewer_agent` (in-pair) | §12 as the presentation-quality screen |
| `formatter_agent` | §4 numeric and P-value formatting consistency against the venue profile |
| `citation_compliance_agent` | §4 (statistic-reporting completeness), §11 (bounded priority claims) |
