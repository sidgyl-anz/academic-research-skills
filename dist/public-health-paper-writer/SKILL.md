...
# Public Health Paper Writer — Academic Paper Writing Agent Team

> **Standalone public-health build.** This is the `academic-paper` writing
...
> (it will still write a non-PH paper), but absent a clear signal it assumes a
> public-health manuscript and applies `references/public_health_writing_standards.md`.
...
cross-model verifier. **On Claude.ai none of that runs**, so those layers
degrade to advisory prose — the writing is driven entirely by the instructions
in this skill and its references. Specifically:

- **Deterministic gates degrade to advisory.** Mentions of `scripts/*.py`
  (sprint-contract / patch / citation / conformance checkers), PreToolUse
  write-scope guards, and CI workflows describe the Claude Code enforcement
  path. Here the agents follow the same *logic* as instructions; there is no
  separate automated checker.
- **`ARS_*` environment variables are inert** (`ARS_CROSS_MODEL`,
  `ARS_MODEL_TIERING`, `ARS_PASSPORT_RESET`, etc.) and have no effect.
- **File output.** `format-convert` to DOCX/PDF relies on Pandoc/LaTeX being
  available. On Claude.ai, prefer Markdown or LaTeX source output; a `.docx`/
  `.pdf` is only produced if the code-execution tool with Pandoc is enabled.
- **Not every referenced path is bundled.** `./shared/` carries the files the
  writing directly needs (style calibration, handoff/severity schema,
  writer/evaluator + patch + submission contracts, word-count / hedging /
  claim-strength references, disclosure policy data). Deeper pointers — sibling
  skills (`deep-research`, `academic-paper-reviewer`, `academic-pipeline`),
  repo paths (`scripts/`, `docs/design/`, `.claude/CLAUDE.md`), and
  toolkit-only `shared/…` files (Material Passport schemas) — are provenance
  references, **not runtime loads**. If one is absent, ignore it and proceed.
- **What fully works here:** the writing pipeline, public-health structure &
  standards, plan/outline/revision/abstract/lit-review/disclosure modes,
  bilingual structured abstracts, citation formatting, and the templates — all
  prose-driven.

## Public Health Default (this build)

- `intake_agent` still asks for paper type, discipline, citation format, and
  output format. When the topic is public-health-adjacent **or the domain is
  ambiguous**, it defaults the discipline to public health, picks the
  applicable EQUATOR reporting guideline, and every downstream agent loads
  `references/public_health_writing_standards.md`.
- The **outline is guideline-driven**: STROBE/CONSORT/PRISMA (etc.) checklists
  shape the Methods and Results sections so no required item is silently
  omitted.
- **Causal language is calibrated to the design** (association vs causation) and
  the abstract/discussion conclusion never exceeds the results — a standing
  drafting discipline, reinforced by the protected-hedges and claim-strength
  references.
- Abstracts default to the **structured** public-health form (Background/
  Objectives · Methods · Results · Conclusions) in both languages.
- If the topic is clearly NOT public health, announce the detected discipline
  once and write on the general references (the PH standard simply does not
  fire). The default general reference domain remains higher education.

A general-purpose academic paper writing tool — 12-agent pipeline covering all disciplines, tuned in this build to public health by default (higher education remains the fallback general reference).
